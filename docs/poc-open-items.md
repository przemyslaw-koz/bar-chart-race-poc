...
Checklist of known gaps, risks, and follow-up work for the Remotion POC (`app/`) and the planned hybrid cloud pipeline ([ADR](./adr-hybrid-cloud-video-rendering-pipeline.md), [architecture overview](./hybrid-cloud-video-rendering-pipeline.md)).
...
## Remotion app (`app/`) — remaining items
...
Only `app/.gitignore` exists. If the Git root is `bar-chart-race-poc/`, add a root `.gitignore` (e.g. `app/node_modules/`, `app/out/`, `app/dist/`) so `git add` from the root cannot accidentally stage build artifacts.
...
These are **out of scope** for `app/` but required for the architecture in the ADR:
...
1. **Repo hygiene** — root `.gitignore`, remove nested `app/.git` if present, first commit with full `app/` tree (not a gitlink).
...
- [app/README.md](../app/README.md) — local dev, Docker render, input props, benchmark
