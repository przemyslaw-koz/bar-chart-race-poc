...
## Phase 2 — Hybrid Cloud MVP
Goal:
- thin client uploads input JSON to S3
- thin client sends job to SQS
- one EC2 Spot worker picks the job and renders it
- output video lands in S3
- thin client downloads the result

Deliverables:
- Terraform for S3 + SQS + IAM + EC2
- worker bootstrap / AMI
- basic job status tracking
- working end-to-end render

---

## Phase 3 — Reliability & FinOps
Goal:
- make the pipeline resilient and cheap
...
Goal:
- determine whether GPU is worth it

Benchmark:
- thin client vs CPU Spot
- CPU Spot vs GPU Spot
...
## Phase 5 — Portfolio / Production Polish
Potential additions:
...
- Lambda-based rendering experiment for comparison
